...
async def place_order(payload: PlaceOrderRequest, container: Container = Depends(get_container), _: str = Depends(verify_token)) -> dict:
    try:
...
async def list_orders(
    status: str | None = Query(default=None), strategy_id: str | None = Query(default=None), container: Container = Depends(get_container), _: str = Depends(verify_token)
) -> list[dict]:
    return await container.execution_service.list_orders(status=status, strategy_id=strategy_id)


@router.get('/orders/{order_id}', response_model=OrderOut)
async def get_order(order_id: str, container: Container = Depends(get_container), _: str = Depends(verify_token)) -> dict:
    try:
...
async def cancel_order(order_id: str, container: Container = Depends(get_container), _: str = Depends(verify_token)) -> dict:
    try:
...
async def list_trades(
    strategy_id: str | None = Query(default=None), symbol: str | None = Query(default=None), container: Container = Depends(get_container), _: str = Depends(verify_token)
) -> list[dict]:
