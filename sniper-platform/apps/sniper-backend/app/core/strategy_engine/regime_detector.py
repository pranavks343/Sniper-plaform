...
class HMMRegimeDetector:
    def __init__(self, lookback: int = 100) -> None:
        ...
        self.model = GaussianHMM(n_components=3, covariance_type='diag', n_iter=100) if GaussianHMM else None
        self._trained = False
    ...
    def train(self, historical_prices: np.ndarray) -> None:
        ...
        for i in range(self.lookback, len(prices)):
            try:
                windows.append(self.calculate_features(prices[i - self.lookback : i]))
            except ValueError:
                continue
        if not windows:
    ...
    def predict_regime(self, recent_prices: np.ndarray) -> RegimeState:
        features = self.calculate_features(np.asarray(recent_prices, dtype=float)).reshape(1, -1)
        ...
        mapping = {0: Regime.TRENDING, 1: Regime.MEAN_REVERTING, 2: Regime.VOLATILE}
        return RegimeState(regime=mapping.get(state_idx, Regime.MEAN_REVERTING), confidence=confidence, transition_prob=transition_prob)
