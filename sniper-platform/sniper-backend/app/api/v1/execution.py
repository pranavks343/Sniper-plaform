...
async def place_order(payload: PlaceOrderRequest, container: Container = Depends(get_container)) -> dict:
    try:
        return container.execution_service.place_order(payload.model_dump())
...
async def list_orders(
    ...
    return container.execution_service.list_orders(status=status, strategy_id=strategy_id)


@router.get('/orders/{order_id}', response_model=OrderOut)
async def get_order(order_id: str, container: Container = Depends(get_container)) -> dict:
    try:
        return container.execution_service.get_order(order_id)
...
async def cancel_order(order_id: str, container: Container = Depends(get_container)) -> dict:
    try:
        return container.execution_service.cancel_order(order_id)
    except KeyError as exc:
        raise HTTPException(status_code=404, detail='order not found') from exc
...
async def list_trades(
    ...
    return container.execution_service.list_trades(strategy_id=strategy_id, symbol=symbol)
