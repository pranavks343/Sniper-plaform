...
class CircuitBreaker:
    ...
    def check_triggers(self, market_state: dict, portfolio_state: dict) -> bool:
        if abs(portfolio_state.get('daily_loss_pct', 0.0)) > portfolio_state.get('max_daily_loss_pct', 0.02):
            return True
        if abs(portfolio_state.get('drawdown_pct', 0.0)) > portfolio_state.get('max_drawdown_pct', 0.10):
            return True
    ...
    def activate_breaker(self, reason: str) -> None:
        self._status = BreakerStatus(
            ...
            actions_taken=['cancel_pending_orders', 'disable_new_orders', 'snapshot_audit_state'],
        )
